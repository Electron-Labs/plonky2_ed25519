...
use plonky2_sha512::gadgets::sha512;
...
pub fn verify_using_preprocessed_sha_block<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    sha_preprocessed: &Vec<BoolTarget>,
    pub_key: &Vec<BoolTarget>,
    sig: &Vec<BoolTarget>,
) {
    let digest = sha512::sha512_circuit_with_preprocessed_input(builder, sha_preprocessed);
